...
import { logMood, queryMood, type MoodLogCreate, type MoodLogItem } from '@/features/trackers/api';
...
export default function MoodPage() {
  ...
  const [logs, setLogs] = useState<MoodLogItem[]>([]);
  ...
  const [avgMood5, setAvgMood5] = useState<number>(0);
  ...
  const fetchLogs = async (opts?: { keepLoading?: boolean }) => {
    try {
      ...
      const data = await queryMood(params);
      setLogs(data);
    } catch (e) {
      console.error(e);
      const m = mapApiError(e);
      show(m.message || 'Failed to load mood logs', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    const fetchSummary = async () => {
      try {
        ...
        const data = await queryMood({ from: start.toISOString(), to: now.toISOString(), limit: 500 });
        const buckets: Record<string, { count: number; sumNorm5: number }> = {};
        for (let i = 0; i < 7; i++) {
          const d = new Date(start);
          d.setDate(start.getDate() + i);
          const key = d.toISOString().slice(0, 10);
          buckets[key] = { count: 0, sumNorm5: 0 };
        }
        data.forEach(m => {
          const key = new Date(m.when).toISOString().slice(0, 10);
          if (!buckets[key]) buckets[key] = { count: 0, sumNorm5: 0 };
          buckets[key].count += 1;
          const scale = m.scale || 5;
          const norm5 = (m.val / Math.max(1, scale)) * 5;
          buckets[key].sumNorm5 += norm5;
        });
        const counts: number[] = Object.keys(buckets).sort().map(k => buckets[k].count);
        setDailyCounts(counts);
        setTotalEntries(data.length);
        const overallAvg = data.length
          ? data.reduce((acc, m) => acc + (m.val / Math.max(1, (m.scale || 5))) * 5, 0) / data.length
          : 0;
        setAvgMood5(Number(overallAvg.toFixed(1)));
      } catch (e) {
        console.error(e);
        const m = mapApiError(e);
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const body: MoodLogCreate = {
      when: new Date(String(fd.get('when') || new Date().toISOString())).toISOString(),
      val: Number(fd.get('val') || 3),
      scale: fd.get('scale') ? Number(fd.get('scale')) : 5,
    ...
    try {
      setSubmitting(true);
      await logMood(body);
      setLogs(await queryMood({ limit: 10 }));
      (e.target as HTMLFormElement).reset();
      show('Mood logged', 'success');
    } catch (e) {
      console.error(e);
      const m = mapApiError(e);
      show(m.message || 'Failed to log mood', 'error');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="min-h-[calc(100vh-8rem)] w-full">
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 py-8">
        <div className="max-w-2xl mx-auto space-y-6">
          <div>
            <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">Mood</h1>
            <p className="text-sm text-gray-600 dark:text-gray-400">Daily mood check-in.</p>
          </div>
            ...
            {summaryLoading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-center">
                ...
                <div>
                  ...
                  <div className="text-xl font-semibold text-gray-900 dark:text-white">{avgMood5}/5</div>
                </div>
          ...
          <form onSubmit={onSubmit} className="space-y-3 bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <input name="val" type="number" min={1} max={5} placeholder="Val (1-5)" className="input" required />
              <input name="scale" type="number" min={3} max={10} placeholder="Scale (default 5)" className="input" />
          ...
          <div>
            ...
            {loading ? (
              ...
              <EmptyState title="No mood logs yet" subtitle="Log a check-in to see it here." />
            ) : (
              <div className="space-y-2">
                {logs.map((l) => (
                  <div key={l.id} className="text-sm text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 p-2 rounded">
                    {new Date(l.when).toLocaleString()} — {l.val}/{l.scale} {Array.isArray(l.tags) && l.tags.length ? `· ${l.tags.join(', ')}` : ''}
                  </div>
                ))}
                <div className="pt-2">
                  <button
                    ...
                    aria-label="Load more mood logs"
                  >
                    Load more
                  </button>
                </div>
