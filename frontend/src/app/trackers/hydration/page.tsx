...
import { logHydration, queryHydration, type HydrationLogCreate, type HydrationLogItem } from '@/features/trackers/api';
...
export default function HydrationPage() {
  ...
  const fetchLogs = async (opts?: { keepLoading?: boolean }) => {
    try {
      ...
      const data = await queryHydration(params);
      setLogs(data);
    } catch (e) {
      console.error(e);
      const m = mapApiError(e);
      show(m.message || 'Failed to load hydration', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    const fetchSummary = async () => {
      try {
        ...
        const data = await queryHydration({ from: start.toISOString(), to: now.toISOString(), limit: 500 });
        const buckets: Record<string, { count: number; ml: number }> = {};
        for (let i = 0; i < 7; i++) {
          const d = new Date(start);
          d.setDate(start.getDate() + i);
          const key = d.toISOString().slice(0, 10);
          buckets[key] = { count: 0, ml: 0 };
        }
        data.forEach(h => {
          const key = new Date(h.when).toISOString().slice(0, 10);
          if (!buckets[key]) buckets[key] = { count: 0, ml: 0 };
          buckets[key].count += 1;
          buckets[key].ml += h.amount_ml ?? 0;
        });
        const counts: number[] = Object.keys(buckets).sort().map(k => buckets[k].count);
        setDailyCounts(counts);
        setTotalDrinks(data.length);
        setTotalMl(data.reduce((acc, h) => acc + (h.amount_ml ?? 0), 0));
      } catch (e) {
        console.error(e);
        const m = mapApiError(e);
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const body: HydrationLogCreate = {
      when: new Date(String(fd.get('when') || new Date().toISOString())).toISOString(),
      amount_ml: Number(fd.get('amount_ml') || 0),
    };
    try {
      setSubmitting(true);
      await logHydration(body);
      setLogs(await queryHydration({ limit: 10 }));
      (e.target as HTMLFormElement).reset();
      show('Hydration logged', 'success');
    } catch (e) {
      console.error(e);
      const m = mapApiError(e);
      show(m.message || 'Failed to log hydration', 'error');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="min-h-[calc(100vh-8rem)] w-full">
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 py-8">
        <div className="max-w-2xl mx-auto space-y-6">
          <div>
            <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">Hydration</h1>
            <p className="text-sm text-gray-600 dark:text-gray-400">Record water intake.</p>
          </div>

          {/* Filters */}
          <form
            aria-label="Filter hydration entries"
          ...
          <div className="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
            ...
            {summaryLoading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-center">
                <div>
                  <div className="text-xs text-gray-500 dark:text-gray-400">Drinks</div>
                  <div className="text-xl font-semibold text-gray-900 dark:text-white">{totalDrinks}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500 dark:text-gray-400">Total ml</div>
                  <div className="text-xl font-semibold text-gray-900 dark:text-white">{totalMl}</div>
                </div>
          ...
          <form onSubmit={onSubmit} className="space-y-3 bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <input name="amount_ml" type="number" placeholder="Amount (ml)" className="input" required />
            </div>
          ...
          <div>
            ...
            {loading ? (
              ...
              <EmptyState title="No hydration entries yet" subtitle="Log your water intake to see it here." />
            ) : (
              <div className="space-y-2">
                {logs.map((l) => (
                  <div key={l.id} className="text-sm text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 p-2 rounded">
                    {new Date(l.when).toLocaleString()} — {l.amount_ml} ml
                  </div>
                ))}
                <div className="pt-2">
                  <button
                    ...
                    aria-label="Load more hydration entries"
                  >
                    Load more
                  </button>
                </div>
