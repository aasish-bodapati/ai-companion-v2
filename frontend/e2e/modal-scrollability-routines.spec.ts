...
test.describe('Modal Scrollability - Routines Tab', () => {
  test('should test modal scrollability in routines tab', async ({ page }) => {
    ...
    const routinesTab = page.locator('button:has-text("My Routines")').first();
    await routinesTab.click();
    await page.waitForTimeout(1000);
    
    console.log('Switched to Routines tab');
    ...
    await modal.screenshot({ path: 'modal-routines-test.png' });
  });
});
