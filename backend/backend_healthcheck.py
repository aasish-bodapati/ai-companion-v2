...
class BackendHealthCheck:
    ...
    def _make_request(
        self, method: str, endpoint: str, data: Optional[Dict] = None, auth_required: bool = True
    ) -> Tuple[bool, Dict]:
    ...
    def test_list_conversations(self) -> bool:
        ...
        if success and isinstance(result, list):
            ...
            self._print_result(
                "List Conversations", False, result.get("error", "Invalid response format")
            )
            return False
    ...
    def test_list_messages(self) -> bool:
        ...
        success, result = self._make_request(
            "GET", f"/api/v1/conversations/{self.conversation_id}/messages"
        )
        ...
        if success and isinstance(result, list):
            ...
            self._print_result(
                "List Messages", False, result.get("error", "Invalid response format")
            )
            return False
    ...
    def test_chat_endpoint(self) -> bool:
        ...
        success, result = self._make_request(
            ...
            "/api/chat",
            {
                "messages": [{"role": "user", "content": "Hello, this is a health check message"}],
        ...
        if success and "response" in result and "content" in result["response"]:
            ...
            self._print_result(
                "Chat Endpoint", False, result.get("error", "Invalid response format")
            )
            return False
