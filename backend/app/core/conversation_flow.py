...
class ConversationFlowAnalyzer:
    ...
    def _analyze_engagement_level(self, messages: List[Dict]) -> str:
        ...
        recent_user_messages = [msg for msg in messages[-3:] if msg.get("role") == "user"]
    ...
    def _generate_follow_up_opportunities(
        self, messages: List[Dict], topic_transitions: List[str], engagement_level: str
    ) -> List[str]:
        ...
        if "goals" in topic_transitions or any(
            kw in content for kw in self.topic_keywords["goals"]
        ):
