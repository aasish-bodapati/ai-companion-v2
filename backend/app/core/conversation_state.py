...
class ConversationStateManager:
    ...
    def get_follow_up_opportunities(self, conversation_id: str) -> List[str]:
        ...
        for goal in state.ongoing_goals:
            if (
                goal not in state.recent_mentions
                or (now - state.recent_mentions[goal]).total_seconds() > 86400
            ):  # 24 hours
    ...
    def should_show_proactive_engagement(self, conversation_id: str) -> bool:
        ...
        return (
            len(state.ongoing_goals) > 0
            or len(state.current_themes) > 2
            or state.energy_level in ["medium", "high"]
        )
    ...
    def cleanup_old_states(self):
        ...
        for conv_id, state in self._states.items():
            ...
            last_activity = (
                max(state.recent_mentions.values())
