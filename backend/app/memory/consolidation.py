...
class MemoryConsolidationService:
    ...
    def _find_similar_memories(self, memories: List, threshold: Optional[float] = None) -> List[List]:
        ...
        try:
            ...
            for i, m in enumerate(memories):
                ...
                for j in range(i + 1, len(memories)):
                    if j in used:
                        continue
                    sim = self._calculate_similarity(vecs[i], vecs[j]) if i < len(vecs) and j < len(vecs) else 0.0
                    if sim >= thr:
