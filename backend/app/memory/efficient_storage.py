...
class EfficientMemoryStorage:
    ...
    def retrieve_memory(self, memory_id: str) -> Optional[Dict[str, Any]]:
        ...
        cursor = self.db_conn.execute(
            "SELECT chunk_id FROM memory_index WHERE memory_id = ?", (memory_id,)
        )
    ...
    def _update_memory_index(self, memory_id: str, chunk_id: str, memory_data: Dict[str, Any]):
        ...
        compressed_size = (
            len(memory_data.get("compressed_data", b""))
            if memory_data.get("compressed_data")
            else original_size
        )
    ...
    def search_memories(
        self, query: str, categories: Optional[List[str]] = None, limit: int = 10
    ) -> List[Dict[str, Any]]:
