...
class ContextualMemoryRetriever:
    ...
    def _analyze_conversation_context(
        ...
        context["continuation_signals"] = [
            word for word in continuation_words if word in current_lower
        ]
    ...
    def _build_contextual_query(
        ...
        if conversation_context["current_themes"]:
            ...
            for theme in conversation_context["current_themes"]:
                theme_keywords.extend(
                    self.conversation_themes[theme][:3]
                )  # Top 3 keywords per theme
        ...
        for memory in memories:
            ...
            if conversation_context["conversation_depth"] == "deep":
                ...
                if any(
                    ...
                    for word in ["personal", "important", "goal", "dream", "feel"]
                ):
            ...
            if conversation_context["relationship_building"]:
                if any(
                    word in memory_content for word in ["prefer", "like", "love", "enjoy", "value"]
                ):
                    contextual_score += 0.2
    ...
    def _memory_matches_emotional_context(
        self, memory: MemorySearchResult, emotional_state: str
    ) -> bool:
        ...
        try:
            ...
            metadata = (
                json.loads(memory.memory_metadata)
    ...
    def _memory_has_temporal_relevance(
        self, memory: MemorySearchResult, conversation_context: Dict
    ) -> bool:
