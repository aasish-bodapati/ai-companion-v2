...
class DeduplicationService:
    ...
    async def _get_embedding(self, content: str) -> List[float]:
        ...
        if cached and isinstance(cached, dict) and (now - float(cached["timestamp"])) < self._cache_ttl_seconds:
    ...
    async def is_duplicate(self, content: str, user_id: str, db: Session, content_type: str = "message") -> bool:
        try:
            new_vec = await self._get_embedding(content)
            recent = memory_crud.get_user_memories(db, user_id=user_id, content_type=content_type, limit=100)
            if not recent:
                return False
    ...
    async def count_duplicates(self, user_id: str, db: Session, content_type: str = "message") -> int:
        try:
            recents = memory_crud.get_user_memories(db, user_id=user_id, content_type=content_type, limit=50)
    ...
    def _cleanup_cache(self) -> None:
        now = time.time()
        to_delete = [k for k, v in self._embedding_cache.items() if (now - float(v.get("timestamp", 0))) > self._cache_ttl_seconds]
        for k in to_delete:
...
class MemoryDeduplicationService:
    ...
    def check_for_duplicates(
        ...
        try:
            ...
            new_vec = np.array(new_embedding[0])
            max_similarity = 0.0
            most_similar_id = None
            
            for i, existing_vec in enumerate(existing_embeddings):
                similarity = self._cosine_similarity(new_vec, np.array(existing_vec))
