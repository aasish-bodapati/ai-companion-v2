...
def set_auth_cookies(response: Response, access_token: str, request: Optional[Request] = None) -> None:
    """Set httpOnly cookies for authentication.

    In development on localhost, drop the Secure flag so cookies work over HTTP
    without requiring a process restart when .env toggles. In production (non-localhost),
    honor settings.COOKIE_SECURE.
    """
    secure_default = bool(getattr(settings, "COOKIE_SECURE", True))
    secure = secure_default
    try:
        if request is not None:
            origin = (request.headers.get("origin") or "").lower()
            host = (request.url.hostname or "").lower()
            if origin.startswith("http://localhost:") or host in ("localhost", "127.0.0.1"):
                secure = False
    except Exception:
