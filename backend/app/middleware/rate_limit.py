...
class RateLimitStore:
    ...
    def get_bucket(self, key: str, config: RateLimitConfig) -> TokenBucket:
        ...
        if key not in self.buckets:
            ...
            self.buckets[key] = TokenBucket(capacity=config.burst_limit, refill_rate=refill_rate)
    ...
    def cleanup_old_entries(self) -> None:
        ...
        old_buckets = [key for key, bucket in self.buckets.items() if bucket.last_refill < cutoff]
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # Always let CORS preflight pass through unimpeded so CORSMiddleware
        # can attach the proper headers. Preflights should not be rate-limited.
        ...
        if not bucket.consume():
            ...
            return JSONResponse(
                ...
                headers=self._cors_aware_headers(request, {"Retry-After": "60"}),
            )
        ...
        if hourly_count > self.config.requests_per_hour:
            ...
            return JSONResponse(
                ...
                headers=self._cors_aware_headers(request, {"Retry-After": "3600"}),
            )
    ...
    def _cors_aware_headers(self, request: Request, base: Optional[Dict[str, str]] = None) -> Dict[str, str]:
        """Attach CORS headers to short-circuit responses (e.g., 429) so browsers accept them.
        Mirrors CORSMiddleware behavior for allowed origins.
        """
        headers = dict(base or {})
        origin = request.headers.get("origin")
        # Normalize configured origins like in main.py (rstrip '/')
        try:
            allowed = [str(o).rstrip("/") for o in getattr(settings, "BACKEND_CORS_ORIGINS", [])]
        except Exception:
            allowed = []
