...
class CRUDReminders:
    ...
    def get_user_reminders(self, db: Session, *, user_id: str, limit: int = 100) -> List[ReminderModel]:
        q = db.query(self.model).filter(self.model.user_id == user_id).order_by(self.model.created_at.desc())
        if limit:
            q = q.limit(limit)
        return q.all()

    def create_for_user(self, db: Session, *, user_id: str, obj_in: ReminderCreate) -> ReminderModel:
    ...
    def update_for_user(self, db: Session, *, user_id: str, reminder_id: str, obj_in: ReminderUpdate) -> Optional[ReminderModel]:
        db_obj = db.query(self.model).filter(self.model.id == reminder_id, self.model.user_id == user_id).first()
        if not db_obj:
            return None
