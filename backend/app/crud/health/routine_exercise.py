...
class CRUDRoutineExercise(CRUDBase[RoutineExercise, None, None]):
    ...
    def get_by_workout_day(self, db: Session, *, workout_day_id: str) -> List[RoutineExercise]:
    ...
    def get_by_routine(self, db: Session, *, routine_id: str) -> List[RoutineExercise]:
        """Get all exercises for a specific routine (across all workout days)"""
        return db.query(RoutineExercise).join(
            RoutineExercise.workout_day
        ).filter(
            RoutineExercise.workout_day.has(routine_id=routine_id)
        ).order_by(RoutineExercise.order_index).all()
