...
async def stream_calendar_response(
    ...
    try:
        logger.debug("calendar.stream.start conv=%s len=%d", str(conversation_id), len(calendar_response))
        
        # Stream the calendar response character by character
        for char in calendar_response:
            if await client_disconnected():
                logger.debug("Client disconnected during calendar stream conv=%s", str(conversation_id))
                break
            yield f"data: {char}\n\n"
        ...
        logger.error(
            "calendar.stream.error conv=%s error=%s", 
