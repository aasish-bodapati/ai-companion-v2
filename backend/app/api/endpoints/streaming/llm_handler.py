    ...
    try:
        ...
        async for chunk in generate_response_stream(
            ...
            if chunk and chunk.strip():
                ...
                if not first_token_recorded:
                    ft_ms = (time.perf_counter() - t_start) * 1000.0
                    try:
                        from app.main import app as _app

                        store = getattr(_app.state, "llm_latency", None)
                        if store is None:
                            store = {"first_token_ms": [], "llm_total_ms": []}
                            _app.state.llm_latency = store
                        arr = store.setdefault("first_token_ms", [])
                        arr.append(round(ft_ms, 2))
                        if len(arr) > 500:
                            del arr[: len(arr) - 500]
                    except Exception:
                        pass
        ...
        try:
            ...
            if store is None:
                store = {"first_token_ms": [], "llm_total_ms": []}
                _app.state.llm_latency = store
            arr = store.setdefault("llm_total_ms", [])
            arr.append(round(total_ms, 2))
            if len(arr) > 500:
                del arr[: len(arr) - 500]
        except Exception:
            pass

        # Record to core metrics (latency only available here)
        try:
            record_llm_call(
                model=settings.LLM_MODEL_DEFAULT,
                prompt_tokens=None,
        ...
        logger.error(
            "llm.stream.error conv=%s error=%s", str(conversation_id), str(e), exc_info=True
        )
