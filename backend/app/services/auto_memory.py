...
from app.memory.service_mixins_storage import _apply_pii_policy
...
class AutoMemoryService:
    ...
    def _should_skip_message_capture(self, content: str) -> bool:
        ...
        if any(m in text for m in self._meta_prompts):
            if not any(
                p in text for p in ("my ", "i am", "i have", "i like", "i enjoy", "i prefer")
            ):
                return True
        return False
    ...
    def find_similar_memories(
        self, db: Session, user_id: str, content: str, threshold: float = 0.8
    ) -> List[MemoryNode]:
        """Find existing memories similar to new content"""
        try:
            # Use memory service to find semantically similar memories
            existing_memories = memory_crud.get_user_memories(db=db, user_id=user_id, limit=50)
    ...
    def should_consolidate(self, new_content: str, existing_memory: MemoryNode) -> bool:
        ...
        update_indicators = ["now", "current", "latest", "updated", "changed", "new"]
    ...
    def consolidate_memory(
        self, db: Session, new_content: str, existing_memory: MemoryNode
    ) -> MemoryNode:
    ...
    def auto_capture_memory(
        self, db: Session, user_id: str, content: str, context: Dict[str, Any]
    ) -> Optional[MemoryNode]:
        """
        ...
        try:
            # Enforce PII policy first
            sanitized, blocked = _apply_pii_policy(content)
            if blocked:
                logger.info("Sensitive content blocked from auto memory capture")
                return None
            content = sanitized

            # Calculate importance on sanitized content
            ...
            # For all other content, use the standard threshold but be more lenient
                ...
                effective_threshold = min(
                    settings.AUTO_IMPORTANCE_THRESHOLD * 0.8, 0.12
            ...
            if importance >= effective_threshold:
                ...
                similar_memories = self.find_similar_memories(
                    db, user_id, content, threshold=0.7
                )  # Lowered from 0.8

                if similar_memories:
                    # Consolidate with existing memory
                    existing_memory = similar_memories[0]
                    consolidated_content = self.consolidate_memories(
                    ...
                    existing_memory.importance_score = max(
                        existing_memory.importance_score, int(importance * 100)
                    )

                    # Enhanced metadata update
                    try:
                        metadata = (
                            json.loads(existing_memory.memory_metadata)
                            if existing_memory.memory_metadata
                            else {}
                        )
                    except Exception:
                        metadata = {}
                    ...
                    logger.info(
                        f"Consolidated memory for user {user_id}, importance: {importance}, type: {ct}"
                    )
                    return existing_memory
                else:
                    # Create new memory
                    ...
                    logger.info(
                        f"Auto-captured memory for user {user_id}, importance: {importance}, type: {ct}"
                    )
                    return memory
    ...
    def capture_from_message(
        self, db: Session, user_id: str, message: str, conversation_id: Optional[str] = None
    ) -> Optional[MemoryNode]:
