...
class ConversationAPIClient:
    ...
    def chat(self, message: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if "response" in response_data and "content" in response_data["response"]:
                ...
                print("✅ Successfully parsed response from model (fallback format)")
    ...
    def list_conversations(self) -> list:
        ...
        try:
            response = requests.get(f"{self.api_prefix}/conversations/", headers=self.headers)
            response.raise_for_status()
    ...
    def list_messages(self, conversation_id: str) -> list:
        ...
        try:
            response = requests.get(
                f"{self.api_prefix}/conversations/{conversation_id}/messages", headers=self.headers
            )
            response.raise_for_status()
            return response.json()
        except Exception as e:
