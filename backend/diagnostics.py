...
def compare_schemas_models() -> Dict[str, Any]:
    ...
    result = {
        "conversation": {"matches": True, "issues": []},
    ...
    try:
        ...
        for name, field in Conversation.__fields__.items():
            schema_fields.append(
                SchemaField(name=name, type=field._type_display(), required=field.required)
            )

        model_fields = []
        for name, column in ConversationModel.__table__.columns.items():
            model_fields.append(
                ModelField(name=name, type=str(column.type), nullable=column.nullable)
            )
        ...
        for field in schema_fields:
            if field.name not in model_field_names and field.required:
                result["conversation"]["issues"].append(
                    f"Missing required field in model: {field.name}"
                )
        ...
        for field in model_fields:
            if field.name not in schema_field_names and field.name != "id":
                result["conversation"]["issues"].append(
                    f"Extra field in model not in schema: {field.name}"
                )
                result["conversation"]["matches"] = False
...
def capture_error() -> Dict[str, Any]:
    ...
    try:
        ...
        response = requests.get(f"{settings.SERVER_HOST}/api/v1/conversations/", headers=headers)
