  ...
  switch (goal.id) {
    ...
    case 'daily_water_intake':
      ...
      if (userData.gender === 'female') {
        ...
        DebugUtils.log('💧 Water goal customized for female: 2.7L');
      } else if (userData.gender === 'male') {
        customized.targetValue = 3700; // 3.7L for males
        DebugUtils.log('💧 Water goal customized for male: 3.7L');
      } else {
        // Default to 3L for other/unspecified gender
        customized.targetValue = 3000;
        DebugUtils.log('💧 Water goal using default: 3L (gender:', userData.gender, ')');
      }
      break;
